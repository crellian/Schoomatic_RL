...
class CarlaLapEnv(gym.Env):
        ...
        while True:
            try:
                ...
                if self.town:
                    ...
                    self.world = World(self.client, sample(town, 1)[0])
                ...
                print("time-out of " + str(self.timeout) + "000ms while waiting for the simulator, make sure "
                                                           "the simulator is ready and connected to", self.host, ":",
                      self.port)
            else:
                break
    ...
    def reset(self, **kwargs):
        ...
        if self.src_loc:
            self.route_waypoints = compute_route_waypoints(self.world.map,
                                                           self.world.map.get_waypoint(
                                                               carla.Location(x=self.src_loc[0], y=self.src_loc[1])),
                                                           self.goal_wp,
    ...
    def render(self, mode="human"):
        if self.display:
            ...
            self.display.blit(pygame.surfarray.make_surface(self.viewer_image['rgb'].swapaxes(0, 1)),
                              (self.bev_viewer_res[0], 0))
    ...
    def step(self, action):
        ...
        if self.bev_obs:
            ...
            img = np.expand_dims(observation['rgb'], axis=0)
            img = np.transpose(img, (0, 3, 1, 2))
            image_val = torch.tensor(img).to(device) / 255.0
            z = self.resnet(image_val.float())
            self.observation["obs"] = z[0].cpu().detach().numpy()
        ...
        if self.debug:
            ...
            if self.terminal_state:
                dict = {"reward": self.reward_ls, "observation": self.observation_ls,
                        "termination": self.terminal_state_ls, "info": self.info_ls}
    ...
    def _get_observation(self):
        if self.birdview_producer_obs:
            bev_obs = np.expand_dims(self.birdview_producer_obs.produce(agent_vehicle=self.vehicle)[:, :, 0] * 255,
                                     axis=-1)
        else:
    ...
    def _on_collision(self, event):
        if get_actor_display_name(event.other_actor) != "Road" and get_actor_display_name(
                event.other_actor) != "Roadline":
            self.collision = True
            if self.hud:
